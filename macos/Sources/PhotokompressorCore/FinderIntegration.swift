...
/// macOS equivalent of ShellRegistration.cs. Windows adds a per-extension
/// HKCU shell verb; the closest macOS analogue to "right-click a photo ->
/// Compress with Photokompressor" is a Finder Quick Action, which is an
/// Automator .workflow bundle installed to ~/Library/Services (no admin
/// needed, same as the Windows HKCU-only approach — this only ever touches
/// the current user's Library).
///
/// NOTE: this piece was validated against real Apple-shipped reference
/// bundles (/System/Library/Services/*.workflow, e.g. "Show Map.workflow"
...
public enum FinderIntegration {
    ...
    /// `appPathOverride` exists for testing from a non-.app context (e.g. the
    /// CLI target); the real app never passes it and relies on
    /// `runningAppBundlePath()` instead.
    public static func register(appPathOverride: String? = nil) throws {
        ...
        // Lives in Contents/Resources, not Contents itself — confirmed against
        // the real .workflow bundles under /System/Library/Services.
    ...
    private static func runningAppBundlePath() -> String? {
        // Three levels up from the executable inside Contents/MacOS/Photokompressor.
    ...
    private static func infoPlist() -> String {
        ...
        <dict>
            ...
            <string>gr.antidot.photokompressor.quickaction</string>
    ...
    private static func documentWorkflow(appPath: String) -> String {
        // Passes every selected file to the app in one launch — `open -a` with
        // multiple paths delivers them all via a single application(_:open:)
        // call, which is what AppCoordinator's debounce logic expects.
