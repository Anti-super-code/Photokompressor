...
Writes background.png (640x500) and background@2x.png (1280x1000) next to this
script. build/package-mac.sh hands the 1x file to create-dmg (folded with the
2x into a multi-DPI TIFF); the icon coordinates in that script line up with the
arrow and caption drawn here.

Same light ground (#EDF1F6) and Fira Sans Condensed as the app, so the disk
image reads as part of Photokompressor and not like a stock template.
"""
...
FONT = REPO / "macos/Sources/Photokompressor/Resources/Fonts/FiraSansCondensed-SemiBold.ttf"
...
# (READ-ME-FIRST.txt, LICENSE, THIRD-PARTY-NOTICES.md) needs headroom below it
# for its labels, which is what the extra window height buys.
APP_C = (170, 175)
...
def render(scale: int) -> Image.Image:
    ...
    caption = "Drag Photokompressor onto Applications to install"
    ...
    # Label for the second row, sitting just above those icons.
