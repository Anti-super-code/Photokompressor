...
# Photokompressor.app (Apple Silicon only), ad-hoc signed, in a styled .dmg,
...
1. Open this disk image and drag Photokompressor.app onto the Applications
   shortcut (or run it from wherever you put it — it doesn't need to live
   in /Applications). Eject the disk image when you're done.
...
2. Because this build isn't notarized by Apple, the first time you open it
   ...
   malware." Open System Settings > Privacy & Security, scroll to the
   bottom, and click "Open Anyway" next to the Photokompressor line, then
   confirm. (On macOS 14 and earlier you can instead right-click
   Photokompressor.app, choose Open, then Open again.) You only need to do
   this once.
...
# Applications drop-link, the background and the icon layout itself. The libvips
# notices ride along under a short name so its label doesn't get truncated.
SRC="$STAGE-dmg"
rm -rf "$SRC"; mkdir -p "$SRC"
cp -R "$APP" "$SRC/Photokompressor.app"
cp "$STAGE/READ-ME-FIRST.txt" "$SRC/READ-ME-FIRST.txt"
cp "$STAGE/LICENSE" "$SRC/LICENSE"
cp "$STAGE/THIRD-PARTY-NOTICES-libvips.md" "$SRC/THIRD-PARTY-NOTICES.md"
...
rm -f "$DMG"
create-dmg \
    --volname "Photokompressor $VERSION" \
    --volicon "$APP/Contents/Resources/Photokompressor.icns" \
    --background "$DMG_BG" \
    --window-pos 200 120 \
    --window-size 640 500 \
    --icon-size 100 \
    --text-size 14 \
    --icon "Photokompressor.app" 170 175 \
    --app-drop-link 470 175 \
    --icon "READ-ME-FIRST.txt" 128 345 \
    --icon "LICENSE" 320 345 \
    --icon "THIRD-PARTY-NOTICES.md" 512 345 \
    "$DMG" "$SRC"
rm -rf "$SRC" "$DMG_BG"
