...
$version = ([xml](Get-Content (Join-Path $repo 'src\Photokompressor\Photokompressor.csproj'))).
            Project.PropertyGroup.Version | Where-Object { $_ } | Select-Object -First 1
$name = "Photokompressor-$version-win-x64"
$stage = Join-Path $dist $name
$zip = Join-Path $dist "$name.zip"

Write-Host "Packaging Photokompressor $version" -ForegroundColor Cyan
...
Write-Host '-> publish'
dotnet publish (Join-Path $repo 'src\Photokompressor') `
...
2. Move this folder somewhere permanent first - Documents, or
   %LOCALAPPDATA%\Programs\Photokompressor. The right-click menu remembers where
   the app is, so moving it afterwards breaks the menu entry.

3. Run Photokompressor.exe. Windows will warn that the publisher is unknown,
   because the app is not code-signed: choose More info -> Run anyway.

4. Click the round "i" button, then switch on "Right-click menu".
   You can now right-click any photo in Explorer and compress it.
   On Windows 11 the entry sits under "Show more options".

To uninstall: switch "Right-click menu" back off, delete this folder, and delete
%APPDATA%\Photokompressor.
